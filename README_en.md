...
- **Branch-level Locking**: Only one task can execute per branch at a time
...
gh-claude supports AI Agents (like OpenCLAW) to drive code modifications automatically via GitHub.
...
- **PR Trigger**: Use `@claude` or `/claude` in PR review
- **Auto Execute or review comments**: gh-claude automatically creates branches, executes tasks, commits code
